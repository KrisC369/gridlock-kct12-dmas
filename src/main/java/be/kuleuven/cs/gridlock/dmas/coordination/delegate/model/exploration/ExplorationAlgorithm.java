...
public class ExplorationAlgorithm implements IExplorationAlgorithm {
    ...
    @Override
    public Itinerary<NodeReference, VirtualTime> getSequenceOfStations(IVehicleContext vehContext, VirtualTime currentTime)
            throws NoRoutePossibleException {
        if (vehContext == null) {
            throw new IllegalStateException("Not initialized yet");
        }
        NodeReference currentPosition = vehContext.getCurrentPosition();
        NodeReference destination = vehContext.getEV().getIntendedFinalDestination();
        Path route = this.context.getRouting().route(currentPosition, destination, this.context.getGraph());

        ExplorationAnt initialAnt = antbuilder.buildExplorationAnt(currentTime, vehContext, route);
        ...
        for (NodeReference ref : stationsequence) {
            retmap.add(ref, ant.getTimeStamp(ref), ant.getChargeLevelAt(ref), ant.getWaitingTimeAt(ref));
...
//        }
     ...
     * @return 
     */
    private ExplorationAnt chooseBestAnt(List<ExplorationAnt> ants) throws NoRoutePossibleException {
        if (ants == null) {
...
//            if (minant.getTravelTime().getSeconds() < mincost) {
//                minant = a;
//                mincost = minant.getTravelTime().getSeconds();
//            }
        }
        return minant;
    }
