 ...
 */
public class SimpleElecVehicleAgentFactory implements VehicleAgentFactory {
   ...
   @Override
    public void initialize( SimulationContext simulationContext ) {
        ...
        if( this.routingService == null ) {
            Logger.getLogger( SimpleElecVehicleAgentFactory.class.getCanonicalName() ).log( Level.SEVERE, "Could not instantiate a routing service" );
    ...
    public VehicleAgent createAgent( VehicleReference vehicle ) {
        return new SimpleElecVehicleAgent( vehicle, context, routingService, commsMan );
    }
